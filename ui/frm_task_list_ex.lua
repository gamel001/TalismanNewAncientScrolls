...
function layWorld_lbTrackTaskEx_ebTask_OnLDown(self, mouse_x, mouse_y)
   --uiInfo("test IsHyperLink1111::"..tostring(mouse_x)..":"..tostring(mouse_y)); 
    local item = self:PickItem(mouse_x, mouse_y);
    if item==nil or item:IsHyperLink() == false then --item == nil or
       -- uiInfo("test IsHyperLink");
        uiSetEventProcessed(false)
    end
end

function layWorld_lbTrackTaskEx_ebTask_OnLUp(self, mouse_x, mouse_y)
    local item = self:PickItem(mouse_x, mouse_y);
    if item==nil or item:IsHyperLink() == false then --item == nil or
     --   uiInfo("test IsHyperLink");
        uiSetEventProcessed(false)
...
function layWorld_lbTrackTaskEx_ebTask_OnHyperLink(self, hypertype, hyperlink)
     --uiInfo(tostring(hyperlink));
     if hyperlink ~= nil then
        Receive(hyperlink);
    end
end
...
function layWorld_lbTrackTaskEx_Show()
    --local lbTrackTaskEx=uiGetglobal("layWorld.lbTrackTaskEx");
    local ebTask = uiGetglobal("layWorld.lbTrackTaskEx.ebTask");   

    local TaskContent=uiTaskGetTraceTaskContent();
    
    if TaskContent~=nil then
        local ebTaskScrollBar=ebTask:getScrollBarV();
        ebTask:Show();
	local val=ebTaskScrollBar:getValue();	
	
        ebTask:SetRichText(TaskContent,false);
	if val then
	    ebTaskScrollBar:SetValue(val);
	end
    else 
        --
        --ebTask:Hide();
